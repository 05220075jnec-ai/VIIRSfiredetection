...
```text
outputs/fire_mask.npy
outputs/fire_detections.csv
outputs/fire_detections.geojson
outputs/fire_map.html
```
...
```text
data/viirs/snpp/      Suomi NPP VIIRS arrays
data/viirs/noaa20/    NOAA-20 VIIRS arrays
data/viirs/noaa21/    NOAA-21 VIIRS arrays
data/dem/             Bhutan DEM
data/lulc/            Bhutan land-use/land-cover raster or vector
data/buildings/       NSDI Bhutan building footprints
data/vectors/         roads, settlements, dzongs, schools, hospitals, protected areas
```
...
```text
data/dem/*.tif
data/lulc/*.shp, *.geojson, or *.gpkg
data/buildings/*.shp, *.geojson, or *.gpkg
```

With your current files, the run uses:

```text
DEM: data/dem/BHutan_SRTM.tif
LULC: data/lulc/Land Use Land Cover 2020.shp
Buildings: data/buildings/NCRP Building Footprints.shp
```
