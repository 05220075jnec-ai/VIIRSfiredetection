...
from config import PATHS, PROJECT_ROOT, WORKSPACE_ROOT
...
def run_real(
    ...
    fallback_raw = (
        WORKSPACE_ROOT / "data" / "raw" / "viirs" / "historical_workspace"
        if use_fallback_raw
        else None
    )
    ...
    if not observations:
        raise SystemExit(
            "No real VIIRS MOD pairs found in the configured historical VIIRS data folders."
        )
...
def main() -> None:
    ...
    parser.add_argument("--viirs-dir", default=PATHS.viirs, type=Path, help="Folder containing downloaded VIIRS IMG/MOD files.")
    parser.add_argument(
        "--no-fallback-raw",
        action="store_true",
    ...
    parser.add_argument("--out-dir", default=PATHS.outputs, type=Path, help="Output folder.")
    ...
    out_dir = ensure_directory(
        args.out_dir if args.out_dir.is_absolute() else WORKSPACE_ROOT / args.out_dir
    )
    if args.real:
        detections, fire_mask = run_real(
            args.max_observations,
            use_real_context=not args.synthetic_context,
            viirs_dir=args.viirs_dir if args.viirs_dir.is_absolute() else WORKSPACE_ROOT / args.viirs_dir,
